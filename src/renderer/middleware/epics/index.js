...
import {
    ...
    receiveSearchAllProjectsResult,
...
const deleteConfirmEpic = (action$, store) =>
    action$.ofType(CONFIRM_DELETE_PROJECT).map(action => openDeleteConfirmation(action.payload.projectId));
...
const searchAllProjectEpic = (action$, store) =>
    action$
        .ofType(REQUEST_SEARCH_ALL_PROJECTS)
        .do(_ => store.dispatch(fetchingProjects()))
        .switchMap(action => {
            const keyword = action.payload.keyword;
            return api.searchAllProjects(keyword);
        })
        .map(({ data }) => {
            return receiveSearchAllProjectsResult(data);
        });
...
export default createEpicMiddleware(
    combineEpics(
        ...
        deleteProjectEpic,
        searchProjectEpic,
        searchAllProjectEpic,
        reloadProjectsEpic,
        goBackHomeEpic,
        changedProjectListPageHookEpic,
        clearRelatedProjectsEpic
    )
);
